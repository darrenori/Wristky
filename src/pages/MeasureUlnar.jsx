...
const MAX_DEG = 40;
const NORMATIVE = 33.4;

const MSGS = [
  'Keep the phone flat and tilt toward your little-finger side.',
  'Good. Move only at the wrist, not the forearm.',
  'Nice steady ulnar tilt. Hold the path smooth.',
  'Nice ulnar deviation.',
];

export default function MeasureUlnar() {
  ...
  const exerciseContext = getExercisePageContext('MeasureUlnar');
  const nextRoute = getExerciseNextRoute(exerciseContext, 'MeasureEndurance');
  ...
  const finish = useCallback(() => {
    ...
    saveGameResult('ulnar', peakRef.current, exerciseContext);
    setPhase('done');
    setMascotMsg(`Ulnar done. Peak: ${Math.round(peakRef.current)} deg.`);
  ...
  const bobberX = W * 0.56 - (currentAngle / MAX_DEG) * W * 0.18;
      ...
      <div style={layout.stageStyle}>
        <div style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }}>
          <FishingScene
            ...
            bobberX={bobberX}
            bobberY={H * 0.52}
            showFish={phase === 'measuring' ? [{ x: fishX, y: H * 0.515, size: 0.92 }] : []}
            waterLevel={0.48}
            ...
            {phase === 'measuring' && (
              <>
                ...
                <div style={{ background: 'rgba(0,0,0,0.7)', border: '3px solid #5C3317', padding: '8px 12px' }}>
                  <PixelGauge value={currentAngle / MAX_DEG} label="ULNAR" color="#AA44EE" maxDegrees={MAX_DEG} />
                  <div
                    style={{
                      fontFamily: "'Press Start 2P', monospace",
                      fontSize: 7,
